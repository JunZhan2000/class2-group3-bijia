...
async def spider_parser(res):
    ...
    ##获取大标签
    links = selector.xpath('//div[@class="items"][1]/div')
    for link in links:
        number += 1
        try:
            test1 = link.xpath('div[2]/div[2]/a[1]/span[2]/text()')[0]==link.xpath('div[2]/div[2]/a[1]/span[3]/text()')[0]
            if test1 and len(link.xpath('div[2]/div[2]/a[1]/text()[3]')[0])!= 0 :
                name = link.xpath('div[2]/div[2]/a[1]/span[2]/text()')[0] + link.xpath('div[2]/div[2]/a[1]/text()[3]')[0]
            elif test1:
                name = link.xpath('div[2]/div[2]/a[1]/span[2]/text()')[0] + link.xpath('div[2]/div[2]/a[1]/text()[4]')[0]
            else:
                name = link.xpath('div[2]/div[2]/a[1]/span[2]/text()')[0] + link.xpath('div[2]/div[2]/a[1]/text()[3]')[0] + link.xpath('div[2]/div[2]/a[1]/span[3]/text()')[0]
        except:
            name = ''
        price = link.xpath('div[2]/div[1]/div[1]/strong/text()')[0]
        sell_num = link.xpath('div[2]/div[1]/div[2]/text()')[0].replace('人付款','')
        url = link.xpath('div[2]/div[2]/a/@href')[0].replace('//','')
        name = name 
        if_official = False
        shop_name = link.xpath('div[2]/div[3]/div[1]/a/span[2]/text()')[0]
        img_adress = link.xpath('div[1]/div[1]/div[1]/a/img/@data-src')[0].replace('//','')
        url = 'http://'+str(url)
        name = str(name)
        shop_name = str(shop_name)
        img_adress = 'http://'+ str(img_adress)
        price = float(str(price))
        sell_num = sell_num.replace('+','')
        try:
        ...
        ds_name = '淘宝网'
        print(str(number) +':')
        if shop_name != '苏宁易购官方旗舰店' and len(name) >2 and len(url)!= 0 and price>500 :
            ...
            a_baby = Babies(name=name, url=url, sell_num=sell_num, shop_name=shop_name,
			         img_adress=img_adress, ds_name='淘宝网', price=price, if_official=if_official)
            session.add(a_baby)
        else:
            pass
    session.commit()
...
async def spider_search(browser, page,search_baby):
	url = 'https://s.taobao.com/search?q='+search_baby+'&s=0'
	await page.goto(url)
	await page_evaluate(page)
	contents = []
	...
	while page_num < 11:
		...
		url = 'https://s.taobao.com/search?q='+search_baby+'&s={}'.format(44*(page_num))
		await page.goto(url)
